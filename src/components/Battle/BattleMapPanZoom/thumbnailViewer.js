...
function getSVGDocument(objectElem) {
	return objectElem;
...
function bindThumbnail(main, thumb, options) {
	if (!window.main && main) {
		window.main = main;
	}
...
export default function thumbnailViewer(options) {
	var mainViewObjectElem = document.querySelector(options.mainViewId);
	...
	bindThumbnail(main, undefined, options);
	if (options.onMainViewShown) {
		options.onMainViewShown(mainViewSVGDoc, main);
	}

	var thumbViewObjectElem = document.querySelector(options.thumbViewId);
	...
	bindThumbnail(undefined, thumb, options);
