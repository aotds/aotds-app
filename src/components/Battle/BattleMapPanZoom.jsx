...
export default function BattleMapPanZoom({ bogeys }) {
  ...
  const [ map_area, set_map_area ] = useState({ x: 0, y: 0, height: 600, width: 600 });
  ...
  const [ viewbox, set_viewbox ] = useState("0 0 600 600");
    ...
    const zoom_map = (delta) => {
        set_zoom(
            Math.min( 4, Math.max( zoom + (delta / 100), 1 ) )
        );
    };
...
[
    ...
    useEffect( () => { console.log(viewbox) }, [ viewbox ] );

    const drag_map = (e) => {
        set_center( addV( center, e.map( x => -x ) ));
        console.log(e);
    };


  return <div className={ styles.wrapper }>
    <BattleMap bogeys={ bogeys } viewbox={viewbox}
        onDragMap={drag_map}
        onZoomMap={zoom_map}
        />
    ...
    zonebox={ viewbox }
    viewbox={
        `${map_area.x} ${map_area.y} ${map_area.width} ${map_area.height}`
    }
      />
    </div>
  </div>
